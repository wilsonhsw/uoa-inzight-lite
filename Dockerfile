...
ENV LAST_BUILD_DATE "Sun 06 05 23:45:00 NZDT 2018"
...
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 381BA480 \
   ...
   && echo "deb https://cran.stat.auckland.ac.nz/bin/linux/debian jessie-cran34/" | sudo tee -a /etc/apt/R.list \
   && apt-get update -y -q \
   && apt-get upgrade -y -q \
                      r-base \
   && apt-get update -y -q \
